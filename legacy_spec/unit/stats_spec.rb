...
describe Kent, '.job_stats' do
  it "should return a list of the job types in the queue, their counts and the number of each currently running" do
    ...
    old = Time.now - 3600
    DB[:kent_jobs].where(:job_class => "Kent::Job").update(:job_id => 2**33, :error_count => 5, :run_at => old)
