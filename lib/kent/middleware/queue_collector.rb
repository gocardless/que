  ...
  module Middleware
    ...
    class QueueCollector
      ...
      def call(env)
        ...
        # in our queue
        Kent.execute("select * from kent_jobs_summary").each do |labels|
          metric_labels = {
            queue: labels["queue"],
      ...
      def refresh_materialized_view
        ...
        Kent.transaction do
          Kent.execute("set local lock_timeout='100ms';")
          Kent.execute("set local statement_timeout='5000ms';")
          Kent.execute("refresh materialized view kent_jobs_summary;")
          Kent.execute("analyze kent_jobs_summary;")
        end
      rescue StandardError => err
        Kent.logger.info(event: "refresh_materialized_view", error: err.to_s)
      end

      def due_refresh?
        since_analyze = Kent.execute(<<~SQL).first&.fetch("since_analyze")
          select case
            when last_analyze is not null then extract(epoch from now() - last_analyze)
            else null
          end as since_analyze
          from pg_stat_user_tables
          where relname = 'kent_jobs_summary';
        SQL
