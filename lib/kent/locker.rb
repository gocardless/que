  ...
  class Locker
    ...
    def with_locked_job
      ...
      if job
        observe(UnlockTotal, UnlockSecondsTotal) do
          Kent.execute("SELECT pg_advisory_unlock($1)", [job[:job_id]])
        end
      end
    end
    ...
    def execute_lock_job
      ...
      observe(AcquireTotal, AcquireSecondsTotal, strategy: strategy) do
        Kent.execute(:lock_job, [@queue, @cursor]).first
      end
    end

    def exists?(job)
      observe(ExistsTotal, ExistsSecondsTotal) do
        Kent.execute(:check_job, job.values_at(*Job::JOB_INSTANCE_FIELDS)).any? if job
      end
    end
