...
module Kent
  module Adapters
    autoload :ActiveRecord,   "kent/adapters/active_record"
    autoload :ConnectionPool, "kent/adapters/connection_pool"
    autoload :PG,             "kent/adapters/pg"
    autoload :Pond,           "kent/adapters/pond"
    autoload :Sequel,         "kent/adapters/sequel"
    ...
    class Base
      ...
      # Called after Kent has returned its connection to whatever pool it's
      # using.
      def cleanup!; end
      ...
      def execute_prepared(name, params)
        checkout do |conn|
          ...
          if Kent.disable_prepared_statements || in_transaction?
          ...
          begin
            ...
            unless prepared_just_now
              Kent.log level: "warn", event: "reprepare_statement", name: name
      ...
      def cast_result(result)
        ...
        output.map!(&Kent.json_converter)
      end
    end
  end
end
