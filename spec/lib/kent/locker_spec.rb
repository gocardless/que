...
RSpec.describe Kent::Locker do
  ...
  describe ".with_locked_job" do
    before { allow(Kent).to receive(:execute).and_call_original }
    ...
    def expect_to_work(job)
      with_locked_job do |actual_job|
        ...
        expect(Kent).to receive(:execute).
